"""Experiments using 1000 pre-selected genes (Curtis et al)"""
...
filename = "experiments/results/res_1000_D_" + args.task + ".pkl"
try:
...
gene_graphs = [OntologyGraph(neighbors=30, embeddings_name='dl2vec', randomize=False, gene_names=list(features.columns), relabel_genes=False),
...
for gene_graph in gene_graphs:
    ...
    # Create the set of all experiment ids and see which are left to do 'GCN7',
    model_names = [ 'GCN', 'GCN2', 'GCN5', 'GCN3.0', 'GCN4', 'MLP64', 'MLP64_lr4'] #'GCN3','GCN4', 'GCN5', 'MLP64_lr4'] #[model.name for model in models]'MLP64','MLP64_lr4' ] #
    ...
    for row in todo:
        ...
        elif model_name == 'GCN3.0':
            model = GCN(name="GCN_noemb_lay0_chan64_dropout_agg_hierarchical_prepool2", #_lay3_chan64_emb32_dropout_agg_hierarchy", 
                                dropout=True, 
                                cuda=torch.cuda.is_available(),
                                num_layer=1,
                                prepool_extralayers=2,
