"""Experiments with ensemble of classifiers (each centered around different reference gene)"""
...
filename = "experiments/results/ensemble_MB_A_" + args.task + ".pkl"
try:
...
# Load 50 Parker genes
genes_50 = pd.read_csv("data/Parker_centroids.txt", sep="\t")
genes_50 = list(genes_50.IlluminaID)
genes_50 = [g for g in genes_50 if g in (features.columns)]
print(len(genes_50), genes_50)
...
for gene_graph in gene_graphs:
    ...
    search_num_genes=[ 1000, 100] #, 10,200, 500,2000, 4000, 8000, 16000, 20000] #'all'] #, 100, 1000, 2000] #[4000, 8000] #, 8000, 16300]1000,10, 100, 200, 500, 1000, 2000, 4000, 8000, 16000,
    test_size=400
    search_train_size=[100, 1500] #, 50]100, 
    cuda = torch.cuda.is_available()
    trials=3
    search_genes = genes_50
    ...
    model_names = [ 'GCN5', 'GCN3.1', 'GCN', 'GCN16', 'GCN2'] #, 'MLP1','MLP2', 'MLP2.1',] #,  'MLP3', 'MLP4''MLP64_lr4'] #'GCN3','GCN4', 'GCN5', 'MLP64_lr4'] #[model.name for model in models]
    graph_names = [gene_graph.graph_name]
    columns = ["model", "graph", "num_genes", "train_size", "seed"]
    all_exp_ids = [x for x in itertools.product(model_names, graph_names, search_num_genes, search_train_size, range(trials))]
    ...
    for row in todo:
        ...
        models = []
        for i, gene in tqdm(enumerate(search_genes)):
            ...
            elif model_name == 'GCN3.1':
                model = GCN(name="GCN_noemb_lay1_chan32_dropout_agg_hierarchical_prepool2", #_lay3_chan64_emb32_dropout_agg_hierarchy", 
                                    dropout=True, 
                                    cuda=torch.cuda.is_available(),
                                    num_layer=1,
                                    prepool_extralayers=2,
        ...
        try:
            ...
            with torch.no_grad():
                # make predictions
                yhats = [model.predict(x_test).numpy() for model in models]
                yhats = np.array(yhats)
                # sum across ensemble members
                summed = np.sum(yhats, axis=0)
                # argmax across classes
                y_hat = np.argmax(summed, axis=1)
