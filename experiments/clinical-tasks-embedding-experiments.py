"Experiments with various node emebdding methods"
...
filename = "experiments/results/fig-5_MB_withnodeemb_" + args.task + ".pkl"
try:
...
gene_graphs = [OntologyGraph(neighbors=30, embeddings_name='dl2vec', randomize=True, relabel_genes=False), OntologyGraph(neighbors=30, embeddings_name='dl2vec', randomize=False, relabel_genes=False), GeneManiaGraph(relabel_genes=False), StringDBGraph(datastore="./data", relabel_genes=False)] #, OntologyGraph(neighbors=30, embeddings_name='el', randomize=True)] #, OntologyGraph(neighbors=500, embeddings_name='opa2vec', randomize=False)]#, GeneManiaGraph(), StringDBGraph(datastore="./data")] 
# gene_graphs = [StringDBGraph(datastore="./data")]
# gene_graphs = [
...
for gene_graph in gene_graphs:
    ...
    search_num_genes=['all'] #, 100, 1000, 2000] #[4000, 8000] #, 8000, 16300]
    test_size=400
    search_train_size=[100, 1500] #, 50]
    cuda = torch.cuda.is_available()
    trials=3
    search_genes = ["ESR1"]
    node_embeddings = 'dl2vec'

    # Create the set of all experiment ids and see which are left to do'GCN2', 'GCN4', 'GCN5', 'GCN6', 
    model_names = ['GCN', 'GCN2', 'GCN3', 'GCN7', 'MLP64_lr4'] #[model.name for model in models]'MLP64','MLP64_lr4' ] #
    ...
    for row in todo:
        ...
        if model_name == 'GCN':
            model = GCN(name="GCN_lay1_chan64_dropout_agg_hierarchical", #_lay3_chan64_emb32_dropout_agg_hierarchy", 
                                dropout=True, 
                                cuda=torch.cuda.is_available(),
                                num_layer=1,
            ...
            model = GCN(name="GCN_lay1_chan64_dropout_agg_hierarchical_lr=0.0005", #_lay3_chan64_emb32_dropout_agg_hierarchy", 
                                dropout=True, 
                                cuda=torch.cuda.is_available(),
                                num_layer=1,
                                prepool_extralayers=0,
            ...
            model = GCN(name="GCN_lay1_chan16_dropout_agg_hierarchical_prepool2", #_lay3_chan64_emb32_dropout_agg_hierarchy", 
                                dropout=True, 
                                cuda=torch.cuda.is_available(),
                                num_layer=1,
            ...
            model = GCN(name="GCN_lay1_chan16_dropout_agg_hierarchical_prepool2_lr=0.0005", #_lay3_chan64_emb32_dropout_agg_hierarchy", 
                                dropout=True, 
                                cuda=torch.cuda.is_available(),
                                num_layer=1,
                                prepool_extralayers=2,
            ...
            model = GCN(name="GCN_lay1_chan64_agg_hierarchical", #_lay3_chan64_emb32_dropout_agg_hierarchy", 
                                dropout=False, 
                                cuda=torch.cuda.is_available(),
                                num_layer=1,
                                prepool_extralayers=0,
            ...
            model = MLP(name="MLP_lay2_chan64_lr0.001_dropout", cuda=cuda, dropout=True, num_layer=2, channels=64, train_valid_split=0.8, patience=10, lr=0.001)
        elif model_name == 'MLP64_lr4':
            model = MLP(name="MLP_lay2_chan64_lr.0.0001_nodropout", cuda=cuda, dropout=False, num_layer=2, channels=64, train_valid_split=0.8, patience=10, lr=0.0001)
